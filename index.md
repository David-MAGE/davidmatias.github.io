---
layout: default
title: Home
permalink: /
---

<section class="hero">
  <div class="hero-text">
    <h1>David Matias</h1>
    <p class="tagline">PhD Student in Economics · Rice University</p>
    <p class="tagline">Graduate Student Fellow · Baker Institute Center for Energy Studies</p>
    <p class="fields">Energy Markets · Industrial Organization · Regulatory Economics</p>
    <a class="scroll-cue" href="#about">Scroll to explore <span class="arrow">↓</span></a>
  </div>
  <div class="hero-photo">
    <img src="/assets/img/DM.JPG" alt="David Matias">
  </div>
</section>

<section id="about" class="home-section reveal" markdown="1">

## About me

I am a PhD Student in Economics at [Rice University](https://www.rice.edu/) and a Graduate Student Fellow at the [Baker Institute's Center for Energy Studies](https://www.bakerinstitute.org/expert/david-matias-garcia). My research focuses on Energy Markets, Industrial Organization, and Regulatory Economics.

Prior to my doctoral studies, I worked for three years at the [Inter-American Development Bank](https://www.iadb.org/en) as an energy economist. I also hold a Master's in Economics from [El Colegio de México (COLMEX)](https://www.colmex.mx/en) and a Bachelor's degree from the [Universidad Nacional Autónoma de México (UNAM)](https://www.unam.mx/).

<a href="/assets/cv2026.pdf" target="_blank" rel="noopener">Curriculum Vitae (PDF, updated 2026)</a>

</section>

<section id="research" class="home-section reveal" markdown="1">

## Research

Selected work — all publications predate my doctoral studies, primarily from my time at the IDB.

- **Powering Through Drought: The Impact of Water Scarcity on Electricity Generation** (Working paper, 2025)  
  Droughts cut hydro output ~20% in Chile and push thermal generation and emissions up sharply — with long-run lock-in of carbon-intensive capacity. [Publication page](http://dx.doi.org/10.18235/0013848)

- **The role of renewables in smoothing the impact of oil and gas price shocks on inflation** (*Resources Policy*, 2025)  
  Higher renewable shares reduce the pass-through of oil price shocks to inflation across 18 LAC countries. [Publication page](https://doi.org/10.1016/j.resourpol.2025.105577)

- **Green or Growth? Understanding the Relationship between Economic Growth and CO₂ Emissions** (Working paper, 2024)  
  Income elasticities of CO₂ emissions across 136 countries: the Environmental Kuznets Curve holds globally, but not for Latin America and the Caribbean. [Publication page](http://dx.doi.org/10.18235/0012943)

<p><a class="view-all" href="/research/">View all publications & abstracts →</a></p>

</section>

<section id="teaching" class="home-section reveal" markdown="1">

## Teaching

**Rice University**

**Teaching Assistant**
- ECON 611: Geopolitics of Energy (Graduate Course), Prof. Jim Krane — Summer 2026
- ECON 602: Microeconomics of the Energy Sector (Graduate Course), Prof. Peter Hartley — Fall 2026

</section>

<section id="energy-markets" class="home-section reveal" markdown="1">

## Energy Markets

<div class="em-teaser">
  <img src="/assets/img/nga_keelmen_heaving_in_coals_by_moonlight_1942.9.86.jpg" alt="Keelmen Heaving in Coals by Moonlight, J.M.W. Turner, 1835">
  <div>
    <p>Notes on electricity markets, plus my policy blog writing (in Spanish) on energy in Latin America and the Caribbean.</p>
    <p><a class="view-all" href="/energy-markets/">Explore Energy Markets →</a></p>
  </div>
</div>

</section>

<section id="contact" class="home-section reveal" markdown="1">

## Contact

<div class="kv">
  <div>Email</div><span>davidmatias@rice.edu</span>
</div>

<ul style="list-style:none; padding-left:0; margin-top:16px;">
  <li style="margin:8px 0;"><a href="https://scholar.google.com/citations?user=IzirlNUAAAAJ" style="display:inline-flex; align-items:center; gap:9px;"><svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true" style="flex:none;"><path d="M5.242 13.769L0 9.5 12 0l12 9.5-5.242 4.269C17.548 11.249 14.978 9.5 12 9.5c-2.977 0-5.548 1.748-6.758 4.269zM12 10a7 7 0 1 0 0 14 7 7 0 0 0 0-14z"/></svg> Google Scholar</a></li>
  <li style="margin:8px 0;"><a href="https://www.bakerinstitute.org/expert/david-matias-garcia" style="display:inline-flex; align-items:center; gap:9px;"><svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true" style="flex:none;"><path d="M12 2L2 7v2h20V7L12 2zM4 11v7h2.5v-7H4zm5.2 0v7h2.5v-7H9.2zm5.1 0v7h2.5v-7h-2.5zm5.3 0v7H22v-7h-2.4zM2 20v2h20v-2H2z"/></svg> Baker Institute profile</a></li>
  <li style="margin:8px 0;"><a href="https://www.linkedin.com/in/david-matiasg" style="display:inline-flex; align-items:center; gap:9px;"><svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true" style="flex:none;"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.225 0z"/></svg> LinkedIn</a></li>
</ul>

</section>
